...
const AppCard = ({ item }: { item: any }) => {
  ...
  const linkMap: Record<string, string> = {
    ...
    "access": "https://www.microsoft.com/microsoft-365/access",
    "outlook": "https://outlook.live.com/",
    "teams": "https://www.microsoft.com/microsoft-teams",
    "onenote": "https://www.microsoft.com/microsoft-365/onenote",
    "clipchamp": "https://clipchamp.com/",
    "designer": "https://designer.microsoft.com/",
    "defender": "https://www.microsoft.com/microsoft-365/microsoft-defender",
  };
  const linkUrl = linkMap[item.name.toLowerCase()] || "#";
    ...
    <a href={linkUrl} target="_blank" rel="noopener noreferrer" className="block h-full">
      <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:shadow-lg hover:shadow-blue-100/50 transition-all duration-300 cursor-pointer group hover:-translate-y-1 h-full flex flex-col items-center text-center relative overflow-hidden">
        <div className="mb-5 transform group-hover:scale-110 transition-transform duration-300 relative w-14 h-14">
          <Image src={iconPath} alt={item.name} fill className="object-contain" sizes="56px"/>
        </div>
...
function HomeContent() {
  ...
  const appList = t?.app_list || [];
  const faqList = t?.faq_list || [];
                ...
                <div className="relative bg-white/95 backdrop-blur-xl rounded-[2.5rem] p-5 shadow-2xl shadow-blue-100/50 border border-slate-100 z-10 flex flex-col gap-4 text-left">
                  ...
                  {contactStatus === 'success' ? (
                    <div className="text-center py-8 animate-in zoom-in">
                      ...
                      <p className="font-bold text-green-700 text-xl mb-1">{t.support?.success ? "Success" : "Message Sent!"}</p>
                      <p className="text-sm text-green-600/80">{t.support?.success || "We will reply to your email shortly."}</p>
                    </div>
                  ) : (
                    <div className="flex flex-col gap-3">
                      ...
                      <div className="flex flex-col sm:flex-row gap-3">
                        ...
                        >
                          {contactStatus === 'loading' ? <Loader2 className="w-5 h-5 animate-spin" /> : <>{t.support?.submit || "Send"} <Send className="w-4 h-4 ml-2" /></>}
                        </Button>
                      </div>
                    </div>
                  )}
                    ...
                    <div className={`relative bg-white/90 backdrop-blur-md rounded-full p-2.5 transition-all duration-500 z-10 flex flex-col sm:flex-row gap-2 h-full ${
                      ...
                      >
                        {trialStatus === 'loading' ? (
                          ...
                          <>{t.plans.trial_7d} <ArrowRight className="w-4 h-4 ml-2" /></>
                        )}
                      </Button>
                    </div>
                  </div>
        ...
        <section id="faq" className="py-24 relative bg-white border-t border-slate-100">
          <div className="max-w-7xl mx-auto px-6"> 
            ...
            <div className="grid md:grid-cols-2 gap-6 items-start max-w-5xl mx-auto">
                  ...
                  return (
                    <div 
                      ...
                      className={`border rounded-2xl transition-all duration-300 overflow-hidden ${
                        isOpen ? 'border-blue-200 bg-blue-50/30 shadow-lg shadow-blue-100/50 ring-1 ring-blue-100' : 'border-slate-200 bg-white hover:border-slate-300 hover:shadow-md'
                      }`}
                    >
                      <button
                      ...
                      >
                        <span className={`font-semibold pr-8 text-base transition-colors ${isOpen ? 'text-[#0078D4]' : 'text-slate-800 group-hover:text-[#0078D4]'}`}>
                          {faq.question}
                        </span>
                        <div className={`shrink-0 p-2 rounded-full transition-all duration-300 ${isOpen ? 'bg-[#0078D4] text-white rotate-180' : 'bg-slate-100 text-slate-400 group-hover:bg-blue-50 group-hover:text-blue-500'}`}>
                      ...
                      <div className={`grid transition-all duration-300 ease-in-out ${isOpen ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                        <div className="overflow-hidden">
                          <p className="px-6 pb-6 text-slate-600 leading-relaxed whitespace-pre-line text-sm">
                            {faq.answer}
                          </p>
                        </div>
                      </div>
              ...
              {/* 右列 */}
              <div className="space-y-4">
                {faqList.slice(Math.ceil(faqList.length / 2)).map((faq: any, index: number) => {
