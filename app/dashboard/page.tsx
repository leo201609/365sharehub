...
// 🔥 注意这里：我们不再引入 LanguageProvider，只用 useLanguage
...
function DashboardContent() {
  ...
  const formatMonthYear = (dateString: string) => {
    if (!dateString) return t.common.loading || "Loading...";
  ...
  const handleCheckout = async (planType: string) => {
    ...
    try {
      ...
      else alert(t.common.connection_failed || "Connection failed. Please try again.");
    } catch (error) {
      console.error(error);
  ...
  const getTranslatedPlanName = (planName: string) => {
    ...
    if (planName.includes("Semi-Annual") || planName.includes("6-Months")) return t.plans.semi;
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        ...
        {isPro ? (
          <div className="grid md:grid-cols-3 gap-8 mb-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <Card className="border border-slate-200 shadow-sm h-full bg-slate-50/50">
               ...
               <CardContent className="space-y-5 text-sm">
                  ...
                  <div className="flex justify-between items-center"><span className="text-slate-500">{t.common.email || "Email"}</span><span className="font-medium truncate max-w-[120px] text-slate-900" title={user.email}>{user.email}</span></div>
                  <div className="pt-4">
          ...
          <div className="grid md:grid-cols-3 gap-8 mb-12 max-w-6xl mx-auto items-stretch animate-in fade-in slide-in-from-bottom-8 duration-700">
             ...
             {/* Monthly */}
             <div className="group relative bg-white rounded-3xl border border-slate-200 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 flex flex-col h-full overflow-hidden">
                ...
                <div className="p-8 pt-12 flex flex-col h-full">
                  ...
                  <div className="flex items-baseline mb-6"><span className="text-4xl font-bold text-slate-900">€2.50</span><span className="text-slate-400 ml-1">{t.plans.mo}</span></div>
...
// 🔥 这里是最关键的修复：去掉了 <LanguageProvider> 的包裹！
